...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Retrieve red color component from slider and store in the second element of
		// m_shape_color.
		ImGui::SliderFloat("Green Channel", &m_shape_color.g, 0.0f, 1.0f);

		// Retrieve red color component from slider and store in the third element of
		// m_shape_color.
		ImGui::SliderFloat("Blue Channel", &m_shape_color.b, 0.0f, 1.0f);


		// Retrieve rotation angle from slider and store it in m_shape_rotation
		ImGui::SliderFloat("Rotation", &m_shape_rotation, 0.0f, 6.28f);
...
//----------------------------------------------------------------------------------------
/*
 * Sets the triangle back into its original state
 * (colour, scale, rotation, position)
 */
void A0::reset()
{
		m_shape_color = glm::vec3(1.0f, 1.0f, 1.0f);
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			m_shape_size += m_shape_size_scale_factor;
